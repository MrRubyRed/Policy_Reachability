...
# Quad Params
    ...
    def V_ret(ALL_x):
        ...
        for i in range(len(perms)): #2**num_actions
            ...
            due_tmp = np.min(np.concatenate((due,sess.run(Tt,{states:Snx})),axis=1),axis=1,keepdims=True);
        ...
        return due,opt_actions; #V,opt_actions; 
    ...
    for i in xrange(iters):
        ...
        if(np.mod(i,renew) == 0 and i is not 0): 
            ...
            print "Train Accuracy = " + str(np.float(sum(opaye))/1000000.) + " Test Accuracy = " + str(np.float(sum(opaye_))/1000.)      
...
main(layers1,t_hor,0,1000000,500,0.001,0.99,99,10000,0.0,0);
